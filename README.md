...
2. **Use Interactive CLI**: It's the easiest way to explore
3. **Check Cycles First**: Circular dependencies are often the most important
4. **Export for Visualization**: Use JSON export with visualization tools
5. **Combine with Code Review**: Use this tool alongside manual code review

## Learning Resources

### Understanding the Graph in Neo4j

After loading data, you can query Neo4j directly:

```cypher
// Find all variables
MATCH (v:Variable) RETURN v.name

// Find dependencies of a variable
MATCH (v:Variable {name: "x"})-[:DEPENDS_ON*]->(dep:Variable)
RETURN dep.name

// Find what depends on a variable
MATCH (v:Variable {name: "x"})<-[:DEPENDS_ON*]-(dependent:Variable)
...
For questions or issues, check the code comments or explore the interactive CLI.
